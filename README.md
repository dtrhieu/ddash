...
- Run all tests from repo root:
  - `pytest`
  - verbose: `pytest -v`
- Test locations: `backend/tests/` with filenames matching `test_*.py`.
- Django settings are auto-configured via `pytest.ini` (DJANGO_SETTINGS_MODULE=app.settings, pythonpath=backend).
- Both pytest-style tests and unittest.TestCase-based tests are supported by pytest.
